...
| E6.8 | Manual step entries made offline are lost outright: `api.addSteps` / `bulkSteps` are not registered with the outbox, and `POST /steps` is provenance-aware and append-shaped rather than the idempotent upsert the tracker writes are, so it needs its own coalescing rule | src/api.ts, outbox.ts, server/routes/steps.ts | S | fable | A step entry made offline reaches the server on reconnect | [ ] |
