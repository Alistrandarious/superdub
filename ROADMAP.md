...
| E6.4 | Offline habit ticks are lost silently: optimistic UI, `.catch(() => {})`, no write queue, and the SW never caches /api/. The most-repeated action in the app | src/outbox.ts, api.ts, OutboxBanner.tsx | M | fable | A tick made offline either syncs on reconnect or says it did not land | [x] done v2.493 — durable outbox on both tracker writes (covers the twin on the Progress grid), coalesced per cell, pending pill. **Reads are still online-only**: reloading while offline shows "Couldn't load", so an offline read model is the follow-on. Tasks/journal/goals still lose writes — same outbox, one registration each |
